...
public class Mute
{
  ...
  public Mute(UUID uuid, String reason, String admin, long expireTime)
  {
    this.ID = -1;
    this.uuid = uuid;
    this.reason = reason;
  ...
  public void delete()
  {
    plugin.getMysql().updateNow("DELETE FROM `tools_mutes` WHERE `uuid` = '" + getUuid() + "'");
  }
  
  public void insert()
  {
    String query = "INSERT INTO `tools_mutes` SET `uuid` = '" + getUuid() + "', `admin` = '" + getAdmin() + "', `reason` = '" + getReason() + "', `createTime` = '" + getCreateTime() + "', `expireTime` = '" + getExpireTime() + "'";
    plugin.getMysql().updateNow(query);
  }
  
  public void update(boolean paramBoolean)
  {
    String query = "UPDATE `tools_mutes` SET `admin` = '" + getAdmin() + "', `reason` = '" + getReason() + "', `createTime` = '" + getCreateTime() + "', `expireTime` = '" + getExpireTime() + " WHERE `uuid` = '" + getUuid() + "'";
    if (paramBoolean) {
      plugin.getMysql().updateNow(query);
    } else {
